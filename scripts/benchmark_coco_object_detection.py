...
"""
Benchmarks one compiled TensorRT engine against the COCO val2017 set:
latency, throughput, peak RAM delta, GPU utilization, CO2 (CodeCarbon),
and mAP.
...
parser.add_argument("--metrics_csv", type=str, default="results/coco_detection_one_image.csv")
...
pred_json_path = f"tmp_preds_{clean_model_name}.json"
...
def main():
    ...
    try:
        ...
        for img_id in image_ids:
            try:
                ...
                detections = parse_detections(buffers, output_names, orig_w, orig_h, INPUT_W, INPUT_H)
                for det in detections:
                    coco_results.append({"image_id": int(img_id), **det})

            except Exception as e:
                print(f"⚠️  Skipping image_id={img_id} after a processing error: {e}")
                skipped_images += 1
                continue
    ...
    write_results(coco_results, latency_records, peak_ram_observed, init_ram, emissions_kg,
                  skipped_images, gpu_avg, gpu_peak)


def parse_detections(buffers, output_names, orig_w, orig_h, input_w, input_h):
    if len(output_names) != 3:
        raise RuntimeError(
            f"parse_detections assumes 3 outputs (labels, boxes, scores) but engine has "
    ...
    labels = np.squeeze(buffers[labels_name])
    boxes = np.squeeze(buffers[boxes_name])
    scores = np.squeeze(buffers[scores_name])
...
def write_results(coco_results, latency_records, peak_ram_observed, init_ram, emissions_kg,
                   skipped_images, gpu_avg, gpu_peak):
    mAP, mAP_50 = 0.0, 0.0
    if coco_results:
        coco_gt = COCO(args.annotations)
